...
│   ├── ai_handler.rs    #   /api/ai 系列：provider 增删改查、设为默认、连通性测试、工具调用审计（分页 + 工具名/成败筛选、保留期清理 purge）、GET/PUT /api/ai/crawl-prompt（自定义抓取提示词）、GET/PUT /api/ai/crawl-mode（抓取模式切换）、POST /api/ai/classify-products（同步打标签）、/api/ai/classify-tasks 系列（异步任务：创建/查询/取消）、GET /api/ai/tools（管理 Agent 工具清单）、POST /api/ai/chat（通用管理助手）、/api/ai/chat/sessions 系列（会话：列表/新建/详情/改名/删除/发消息）
...
│   ├── ai/            #   AI 用例：classify_service（自动打标签）/ crawl_agent_service（ReAct 抓取）/ crawl_direct_service（单轮调用抓取，默认）/ crawl_shared（ProductCrawler 端口 + 统计落库共享）/ crawl_switch（运行时按 app_settings 切换两种抓取实现）/ admin_tools（通用管理 Agent 工具集：AdminToolsService + 19 个 AiTool，见下）/ chat_session_service（AI 助手会话：会话+消息持久化，带历史上下文对话）
...
│   ├── ai_conversation.rs # AI 助手会话实体：Conversation（标题自动生成/手动改名）+ ConversationMessage（user/assistant 角色）
│   ├── price_trend.rs   #   PriceTrendPoint/PriceTrendSeries 只读结构（价格趋势图数据，无行为）
│   ├── repository.rs    #   仓储端口 trait：ItemRepository / CrawlTaskRepository / TagRepository / ProductRepository / QueueRepository / AiProviderRepository / AiClassifyTaskRepository / AiToolCallRepository / SettingsRepository（KV 设置）/ ConversationRepository（AI 助手会话）
...
└── src/pages/           #   七个页面：OverviewPage（KPI + QueuesPanel）/ ProductsPage / TagsPage / ItemsPage / TrendsPage / AiAssistantPage / AiPage
```
...
- 前端用 `react-router` 的 **HashRouter**（避免 axum 静态托管需要路由回退）：七个页面 = 概览（KPI + 队列）/ 商品管理 / 标签管理 / 抓取数据 / 价格趋势 / AI 助手 / AI 配置，路由声明在 `App.tsx`，布局壳是 `AppShell.tsx`（antd Layout：Sider 导航 + 48px Header）。Header 常驻队列运行指示（运行中队列进度/排队数），任何页面可感知系统在跑。
...
- **AI 基础设施**（详细方案见 `docs/design-ai-module.md`）：
  ...
  - AI 工具可直接产生读写结果（完全自动，无人工确认）；每次工具执行落 `ai_tool_calls` 审计表（工具名/参数/结果或错误/耗时），前端「AI 工具调用记录」可回查（按工具名/来源/成败筛选）。**来源列（source）**：`domain/ai_tool_call.rs::source` 常量 `assistant`（AI 助手/会话）/ `crawl`（AI 抓取，direct 与 agent 两路径）/ `classify`（AI 打标签）；`AiGateway::run_agent` 带 `source` 参数统一落审计，direct 抓取路径手写审计也标 crawl；老库启动时 ALTER 补 `source TEXT` 列，迁移前记录为 NULL（前端显示「—」）。审计记录只增不改，**不做单条删除**；膨胀控制走保留期清理：`POST /api/ai/tool-calls/purge(/preview)`，条件二选一——`before_days`（删 N 天前，0=清空）或 `keep_latest`（仅保留最新 N 条），交互沿用 preview/confirm 批删模式。
  ...
  - **通用管理 Agent 工具集**（`application/ai/admin_tools.rs`，**19 个 AiTool**）：把后台各接口的能力直接暴露成工具，供 `POST /api/ai/chat`（自然语言 → agent 自主调工具查改数据）与外部智能体使用。工具列表：商品 `list_products`/`get_product`/`create_product`/`update_product`/`delete_product`/`batch_create_products`，标签 `list_tags`/`create_tag`/`update_tag`/`delete_tag`，抓取记录 `list_items`，统计 `get_stats`，队列 `list_queues`/`get_queue`/`enqueue`/`pause_queue`/`resume_queue`/`cancel_queue`，趋势 `get_price_trend`。
    - 设计：工具**直接调 application 层 service**（复用业务规则：重名校验、标签存在校验、队列去重等），不走 HTTP；读工具收敛结果（分页/字段裁剪）防上下文淹没；删除类工具 description 要求先确认。**时间字段一律输出可读形式**（`*_time`，如 `last_crawled_time`/`crawled_time`，格式「YYYY-MM-DD HH:MM」，`fmt_ts` 用 civil-from-days 手写实现不引入 chrono），**不返回原始 Unix 时间戳**，否则 AI 会原样复述时间戳。
    - `AdminToolsService::tools()` 集中注册；`GET /api/ai/tools` 返回 name/description/参数 Schema（JSON，供外部智能体动态注册）；`POST /api/ai/chat` 入参 `{message}`，走 `run_agent` 循环（max_rounds=12），所有工具调用自动落 `ai_tool_calls` 审计。前端「AI 助手」页（`web/src/pages/AiAssistantPage.tsx` + `web/src/components/AiChat.tsx`）内置对话界面：自然语言指令 → 调 `POST /api/ai/chat` → 展示 agent 回答（ReAct 循环在后端跑，前端等最终结果）；未配置 AI 时给出引导。
    - **AI 助手会话系统**（`application/ai/chat_session_service.rs` + 前端 `AiChat.tsx` 会话侧栏）：会话与消息落 `ai_conversations`/`ai_conversation_messages` 表（SQLite，重启不丢）。接口：`GET/POST /api/ai/chat/sessions`、`GET/DELETE /api/ai/chat/sessions/{id}`、`PUT /api/ai/chat/sessions/{id}/title`（改名）、`DELETE /api/ai/chat/sessions/{id}/messages`（清空会话消息，保留会话本身）、`POST /api/ai/chat/sessions/{id}/messages`（发消息：落库用户消息 → `AdminToolsService::chat_with_history` 带最近 20 条历史跑 agent → 落库 AI 回复）。首条消息自动生成会话标题（`Conversation::auto_title_from`）。前端会话侧栏支持新建/切换/改名/删除，消息流本地乐观更新后刷新。
    - **写操作人工确认（`application/ai/tool_approval.rs`，进程内存态）**：会话级写操作确认闸口。会话有两种模式——`normal`（写工具执行前弹框征求用户同意）与 `yolo`（全部放行，无任何干预），切换走 `PUT /api/ai/chat/sessions/{id}/mode`，模式随 `ConversationResponse.mode` 返回。实现：`AiTool` 端口新增 `is_write()` 默认方法（写工具覆盖为 true）；`AiGateway::run_agent` 增加 `approval: Option<Arc<dyn ToolApproval>>` 参数——AI 助手会话传 `Some`（抓取/打标签等无人值守流程传 `None`，不弹确认）；`execute_tool` 在写工具执行前调 `approval.check()`。确认流程：`check()` 在 yolo/已授权时直接放行，否则创建一条待确认审批（`PendingApproval`）并**阻塞等待用户决策**；前端发送期间每 500ms 轮询 `GET /api/ai/approvals/pending?conversation_id=`，有审批则弹框（不可关闭），用户三选一：`POST /api/ai/approvals/{id}/decide` 的 `allow_once`（允许本次）/ `allow_always`（该对话全部允许，之后该工具不再询问）/ `deny`（拒绝，错误回填模型）；决策超时（300s）自动按拒绝处理。`ChatSessionService::chat` 开头 `approval.reset(id)` 清掉上次请求中断遗留的审批。模式不持久化（重启回 normal）。
    - **全局工具启停**（`admin_tools.rs` + 前端 AiChat「可用工具」抽屉）：用户可整体禁用某些工具（影响所有助手会话与 `/api/ai/chat`）。禁用名单存 `app_settings` 键 `ai_tools_disabled`（JSON 数组，缺省=全开）；`AdminToolsService::active_tools()` 每条消息实时过滤（下一条消息生效，无需重启），`tool_manifest()` 返回全部工具并标注 `enabled`/`is_write`。接口：`GET /api/ai/tools`、`PUT /api/ai/tools`（入参 `{disabled_tools}`，未知名拒绝并列出，空数组=全部恢复）。与审批闸口正交：禁用工具不注册进 agent，不触发弹框。
    - 新增工具约定：在 `admin_tools.rs` 里加一个实现 `AiTool` 的 struct（持有对应 service 的 `Arc`），在 `AdminToolsService::tools()` 注册即可；写工具（尤其删除/入队）在 description 里写清副作用，且必须覆盖 `is_write() -> true`（否则确认闸口不会拦截）。
